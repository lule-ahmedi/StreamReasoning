...
public class INWSRDFStreamTestGenerator extends RdfStream implements Runnable {
	...
	public void run() {
            ...
            int n;
            double val = 0;
            double lat = 42.805; double lon = 20.707;  //te rregullohen
...
//                "Glyphosate"
            ...
            while (keepRunning) {
                ...
                //gjate evaluimit perdor kete
                randomWQ = 0; //start from first WQ
//                if(c==10) randomWQ = 14;
//                if(c==20) randomWQ = 13;
//                if(c==30) randomWQ = 14;
                //gjate evaluimit perdor kete
                while(randomWQ<waterQualityArray.length)
                {
                    // n - per numer te individual-it, val - per vlera te observimeve
                    n = (int) randomWithRange(0, 10000);
                    //lokacioni                
                    //location_rnd = (int) randomWithRange(1, ms_number);
                    //gjate evaluimit perdor kete
                    location_rnd = 1;
                    WQ = waterQualityArray[randomWQ];
                    //gjate evaluimit perdor kete
                    while(location_rnd<ms_number)
                    {
                        ...
                        switch(WQ){
                            ...
                            case "WaterHardness": val = randomWithRange(0, 200); break;
                                //plotesoje listen me elementet nga ssn:Property - ... WaterHardness etj.
                        }
...
//                        q = new RdfQuadruple(super.getIRI()+"#obs_" + n, this.ssn + "observedBy", this.inwsCore + "d" + this.c, tempTS);
//                        this.put(q);
//                        System.out.println(q.toString());
                        q = new RdfQuadruple(super.getIRI()+"#obs_" + n, this.inwsCore + "observationResultLocation", this.inwsCore + "ms" + location_rnd , tempTS);
                        this.put(q);
        //                System.out.println(q.toString());
        //                q = new RdfQuadruple(this.inwsCore +"l" + n, this.geo + "lat", lat + "^^http://www.w3.org/2001/XMLSchema#double", tempTS);
        //                this.put(q);
        //                System.out.println(q.toString());
        //                q = new RdfQuadruple(this.inwsCore +"l" + n, this.geo + "long", lon + "^^http://www.w3.org/2001/XMLSchema#double", tempTS);
        //                this.put(q);
        //                System.out.println(q.toString());
        //                q = new RdfQuadruple(super.getIRI()+"#obs_" + n, this.ssn + "featureOfInterest", this.inwsCore + "RiversWaterFeature", tempTS);
        //                this.put(q);
                        //System.out.println("# "+ this.c +": " + "WQ: " +WQ + " Val: " + val +" Loc: " + "ms" + location_rnd + " Time: " + System.currentTimeMillis());
                        //System.out.println();
                        c++;
